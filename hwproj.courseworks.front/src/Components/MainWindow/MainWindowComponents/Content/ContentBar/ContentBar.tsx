...
class ContentBar extends Component<Props, State> {
  constructor(props: Props) {
    ...
    switch (this.props.role) {
      case Roles.Student: {
        this.state = { items: studentContentBarData };
        break;
      }
      case Roles.Lecturer: {
        this.state = { items: teacherContentBarData };
        break;
      }
      case Roles.Curator: {
        this.state = { items: curatorContentBarWorks };
        break;
      }
    }
  }
  ...
  changeBarItem = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    let arr = this.state.items.map((item) => {
      if (item.section === newBarItem) item.selected = true;
